...
{   e.preventDefault();
    ...
    if(checkbox.checked)
        {
            // check();
        let questions=document.querySelectorAll('[name^="q"]');
        let allanswered=true;
        questions.forEach(question=>{
            const options=document.querySelectorAll(`[name="${question.name}"]:checked`);
            if(options.length===0)
            {
                allanswered=false;
            }
        ...
        if(!allanswered)
        {
            alert("please answer all the question before submit");
        }
        else
        {
            check();
...
{   
    ...
    userlistvalue.forEach((el,indx)=>
        ...
        if (percentmark >= 80) {
            ...
            localStorage.setItem("quiztimestamp",new Date().getTime().toString());
